...
impl MemController {
    ...
    pub fn get_mem(&self) -> Result<SetMemory> {
        ...
        m.high = self.get_max_value("memory.high").map_or(Some(MaxValue::Value(0)), |x| Some(x));
        m.low = self.get_max_value("memory.low").map_or(Some(MaxValue::Value(0)), |x| Some(x));
        m.max = self.get_max_value("memory.max").map_or(Some(MaxValue::Value(0)), |x| Some(x));
        m.min = self.get_max_value("memory.min").map_or(Some(MaxValue::Value(0)), |x| Some(x));
